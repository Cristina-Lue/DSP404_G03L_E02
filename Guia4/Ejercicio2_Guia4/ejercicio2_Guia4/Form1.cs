...
namespace ejercicio2_Guia4
{
    public partial class Form1 : Form
    {
        ...
        private void MostrarResultado(double R, int Oper, Boolean hayError = false)
        {
            ...
            if (!hayError)
            {
                IblResul.BackColor = Color.Green;
                IblResul.ForeColor = Color.White;
                IblResul.Text =descripOpe +"es: "+ Convert.ToString(R);
            }
            else
            {
                IblResul.BackColor = Color.Red;
                IblResul.ForeColor = Color.Yellow;
                IblResul.Text = "(ERROR, DIVISION POR CERO)";
            }
        }
